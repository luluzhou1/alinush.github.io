...
In this post, beyond basic group theory for cryptographers[^KL15] and basic polynomial arithmetic, I will assume you are familiar with a few concepts:
 ...
 - **KZG**[^KZG10a] **polynomial commitments** (see [previous post][prevpost]),
    - In particular, can prove $(\phi(a_i) = y_i)_{i\in I}$ using a constant-sized **KZG batch proof** $\pi_I = g^{q_I(\tau)}$, where:
    - $q_I(X)=\frac{\phi(X)-R_I(X)}{A_I(X)}$
    - $A_I(X)=\prod_{i\in I} (X - a_i)$
    - $R_i(a_i) = y_i,\forall i\in I$
        + Interpolated via Lagrange interpolation as $R_I(X)=\sum_{i\in I} y_i \prod_{j\in I,j\ne i}\frac{X - a_j}{a_i - a_j}$
