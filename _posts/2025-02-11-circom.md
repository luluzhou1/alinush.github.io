...
It properly asserts that $2^{N+1}$ fits in a scalar via [assert_bits_fit_scalar](#assertbitsfitscalarn-compile-time-check) (defined later).
...
It also uses [assert_bits_fit_scalar](#assertbitsfitscalarn-compile-time-check) (defined later) and [LessThan](#lessthann-template).
