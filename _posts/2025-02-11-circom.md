...
For example, suppose you were calling `LessThan(2)` on `8` and `1`.
Clearly, `8` is a 4-bit (not 2-bit) number, which will confuse `LessThan(2)` to incorrectly return 1, as if `8 < 1`.
Why?
First, `lhs = 8`, so `1000` in binary.
Second, `rhs = 1`, so `0001` in binary.
Similarly, the mask `1 << N = 1 << 2 = 100`.
So, `lhs + (1 << N) - rhs = 1000 + 100 - 0001 = 1100 - 0001 = 1011`.
Therefore, `LessThan(2)` would incorrectly return 1.
Also, [see here](https://github.com/0xPARC/zk-bug-tracker?tab=readme-ov-file#4-mismatching-bit-lengths).
