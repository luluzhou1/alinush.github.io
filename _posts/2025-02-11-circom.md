...
## Gotchas
 ...
 - The `LessThan` template is loses its soundness when its inputs are `>N` bits: beware and [read this](#lessthann-template)!
...
See an attack below, previously-described at a high-level [here](https://github.com/0xPARC/zk-bug-tracker?tab=readme-ov-file#4-mismatching-bit-lengths):

Suppose, the order of the scalar field is `p`.
 - Assume `N = 2`.
 - Set `lhs = 0` and `rhs = p - 2`.
 - Then, `bits = lhs + (1 << N) - rhs = 2^2 + 2`.
 - So, `out = 1 - bits[N] = 1 - 1 = 0`.
 - Therefore, `LessThan(2)(0, p-2)` would incorrectly output 0[^thanks-dmitry].

If you flip the arguments to `lhs = p-2` and `rhs = 0`, you will also get a soundness break:
 - `bits = lhs + (1 << N) - rhs = p-2 + 2^2 + 0 = p-2 + 4 = 2`.
 - So, `out = 1 - bits[N] = 1 - 0 = 1`, which is incorrect.
   + (It would also stay incorrect for `rhs = 1` and `rhs = 2`.)
...
[^thanks-dmitry]: Big thanks to Dmitry Khovratovich for providing the $(p-2, 0)$ and $(0, p-2)$ counterexamples.
