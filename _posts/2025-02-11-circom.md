...
## Gotchas
 ...
 - Cannot out-of-the-box test that $\ell$ bits "fit" into a $\Zp$ field element, where $p$ is `circom`'s chosen prime
    ...
    + Fortunately, there are [ways to do it manually](#assertbitsfitscalar-compile-time-check).
 - The `LessThan(N)()` template is only sound when its inputs are `N` bits 
    + $\Rightarrow$ it should **not** be used for $a < b$ range checks unless $a$ and $b$ have already had `N`-bit range checks done
    + See [discussion here](https://github.com/0xPARC/zk-bug-tracker?tab=readme-ov-file#4-mismatching-bit-lengths)
...
A good example of this is in the [Num2Bits template](#num2bits-template).
...
```
a === b * q + r;
is_small_remainder <== LessThan(N)([r, b]);
```
...
### `AssertBitsFitScalar()` compile-time check
...
If so, templates like [Num2Bits](#num2bits-template) would have serious soundness issues!

This is why, in this post, I propose checking that **any** `N`-bit number "fits" in circom's configured $\Zp$ scalar type via `AssertBitsFitScalar(N)`.

{: .todo}
Can we make this a _function_, not a _template_?
 ...
 */
template AssertBitsFitScalar(N) {
